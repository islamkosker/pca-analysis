...
def get_image_paths(dir_path, category, sample_size=5):
    all_images = os.listdir(os.path.join(dir_path, category))
    selected_files = random.sample(all_images, sample_size)
    selected_image_paths = [
        os.path.join(dir_path, category, img) for img in selected_files
    ]
    return selected_image_paths, selected_files
...
def top_10_eigenfaces(eigenvectors, prefix="eigenface"):
    top_10_eigenvectors = eigenvectors[:10]  
    eigenfaces = top_10_eigenvectors.reshape((10, 64, 64))  # Reshape to 64x64 for each eigenface
    
    fig, axes = plt.subplots(2, 5, figsize=(15, 6))  # 2 rows, 5 columns of subplots for the top 10 eigenfaces
...
def process_dataset(dir_path, categories, n_components=10, calculate_covariance=True):
    ...
    for category in categories:
        image_paths, selected_files = get_image_paths(dir_path, category)
        selected_files_info[category] = selected_files
...
process_dataset(dir_path, categories, 10, calculate_covariance=False) # To use PCA, set it to false
